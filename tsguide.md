...
Например, если вы используете `@ts-ignore` для подавления ошибок типизации, то будет трудно предсказать, какие типы в конечном итоге будет видеть окружающий код. Для многих ошибок типизации, полезны советы в разделе [как лучше всего использовать](/#any) `any`.
...
При импорте объекта модуля напрямую обращайтесь к свойствам объекта модуля, а не передавайте его. Это гарантирует, что модули могут быть проанализированы и оптимизированы. Отношение к [импорту модулей](/#module-versus-destructuring-imports) как к пространствам имен является нормальным.
...
TypeScript поддерживает два метода организации кода: пространства имен (*namespaces*) и модули, но использование пространств имен необходимо избегать. В google3 в коде должны использоваться TypeScript *модули* (которые являются [модулями ECMAScript 6](http://exploringjs.com/es6/ch_modules.html)). То есть ваш код *должен* ссылаться на код в других файлах с помощью импорта и экспорта вида `import {foo} from 'bar';`
...
Если вам необходима обязательная динамическая (runtime) загрузка для получения сторонних эффектов, используйте `import '...';`. See
...
Встроенный в TypeScript тип `Record<Keys, ValueType>` позволяет создавать типы с определенным набором ключей. Это отличается от ассоциативных массивов тем, что ключи известны статически. См. рекомендации по этому вопросу [ниже](/#mapped-conditional-types).
...
- [Указать более конкретный тип](/#any-specific)
- [Использовать `unknown`](/#any-unknown)
- {Подавить предупреждение анализатора кода и задокументировать, почему](/#any-suppress)
...
Избегайте создания API, которые у которых возвращаемый тип представлен только generic-типом. При работе с существующими API, которые имеют возвращаемый у которых тип представлен только generic-типом, всегда явно указывайте generic-тип.
